...
func main() {
	log.Info().Msg("--> Starting CryptGO-gateway.")
	...
	{
		conn, err := grpc.Dial("CryptGO-match:50051", grpc.WithInsecure())
		if err != nil {
			log.Fatal().Err(err).Msg("--> Did not connect.")
		}
		defer conn.Close()
		matchClient = pb.NewMatchClient(conn)
		log.Info().Msg("--> Connected to CryptGO-match:50051")
	}
	{
		conn, err := grpc.Dial("CryptGO-sql:50051", grpc.WithInsecure())
		if err != nil {
			log.Fatal().Err(err).Msg("--> Did not connect.")
		}
		defer conn.Close()
		sqlClient = pb.NewSqlClient(conn)
		log.Info().Msg("--> Connected to CryptGO-sql:50051")
	}
	{
		conn, err := grpc.Dial("CryptGO-memorystore:50051", grpc.WithInsecure())
		if err != nil {
			log.Fatal().Err(err).Msg("--> Did not connect.")
		}
		defer conn.Close()
		memorystoreClient = pb.NewMemorystoreClient(conn)
		log.Info().Msg("--> Connected to CryptGO-memorystore:50051")
	}
	{
		conn, err := grpc.Dial("CryptGO-wallet:50051", grpc.WithInsecure())
		if err != nil {
			log.Fatal().Err(err).Msg("--> Did not connect.")
		}
		defer conn.Close()
		walletClient = pb.NewWalletClient(conn)
		log.Info().Msg("--> Connected to CryptGO-wallet:50051")
	}
