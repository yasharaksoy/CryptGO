...
func initMessaging() {
	...
	err = consumer.SubscribeTopics([]string{messaging.TopicCryptGOTransfers}, nil)
	if err != nil {
	...
	for {
		...
		case ev := <-consumer.Events():
			...
			case *kafka.Message:
				...
				case messaging.TopicCryptGOTransfers:
					log.Debug().Str("value", string(e.Value)).Msg("receive a CryptGO transfer message")
					ch <- e.Value
				default:
