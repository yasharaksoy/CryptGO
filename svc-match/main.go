...
func (s *server) UpdateBalance(ctx context.Context, in *pb.UpdateBalanceRequest) (*pb.UpdateBalanceResponse, error) {
	...
	producer.ProduceChannel() <- &kafka.Message{TopicPartition: kafka.TopicPartition{Topic: &topicSCryptGOBalances, Partition: kafka.PartitionAny}, Value: b, Key: []byte(key)}
	...
	producer.ProduceChannel() <- &kafka.Message{TopicPartition: kafka.TopicPartition{Topic: &messaging.TopicCryptGOTransfers, Partition: kafka.PartitionAny}, Value: b}
	return response, nil
}
...
func main() {
	log.Info().Msg("--> Starting CryptGO-match")
