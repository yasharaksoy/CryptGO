...
var (
	...
	topicOrders        = "orders"
	topicDeals         = "deals"
	topicSCryptGOBalances = "CryptGO-balances-v1"
	topicCryptGOOrders   = "CryptGO-orders-v1"
)
...
func loadFromKafka() {
	...
	err = consumer.SubscribeTopics([]string{topicSCryptGOBalances}, func(c *kafka.Consumer, event kafka.Event) error {
	...
	for run == true {
		...
		case *kafka.Message:
			...
			case topicSCryptGOBalances:
				log.Debug().Str("--> Key", string(e.Key)).Str("value", string(e.Value)).Msg("receive a CryptGO balances message.") 
	...
	err = consumer.SubscribeTopics([]string{topicCryptGOOrders}, func(c *kafka.Consumer, event kafka.Event) error {
		log.Info().Msgf("--> Rebalanced: %s", event)
		switch e := event.(type) {
		case kafka.AssignedPartitions:
			for i := range e.Partitions {
	...
	for run == true {
		...
		case *kafka.Message:
			...
			switch *e.TopicPartition.Topic {
			case topicCryptGOOrders:
				log.Debug().Str("--> Key", string(e.Key)).Str("value", string(e.Value)).Msg("receive a CryptGO orders message.") 
