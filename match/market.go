...
func orderPut(real bool, m *currencyPair, o *order) {
	...
	if real {
		producer.ProduceChannel() <- &kafka.Message{TopicPartition: kafka.TopicPartition{Topic: &topicCryptGOOrders, Partition: kafka.PartitionAny}, Value: b, Key: []byte(o.Id)}
	}
}
